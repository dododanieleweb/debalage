...
create table if not exists public.events (
  ...
  type             text not null check (type in ('casa_privata','mercatino','mostra','asta')),
...
create table if not exists public.products (
  ...
  subcategory    text,
  condition      text not null default 'good' check (condition in ('mint','excellent','good','fair','poor')),
  era            text,
  brand          text,
  material       text,
  dimensions     text,
